...
- **资产管理模块**：新增资产台账、手动录入、CSV/JSON/nmap 导入、模板下载、资产详情、单个删除与批量删除。
- **资产威胁识别**：结合服务版本、CPE、CVE、KEV、EPSS、暴露面和业务重要性生成风险评分与处置建议。
- **仓库同步策略**：恢复 `docs/` 公开文档同步，开发过程文档归档到本地 `plan/`，测试目录不再同步到 GitHub。
...
- **资产台账**：支持手动录入、CSV/JSON 导入和 nmap XML 解析，沉淀主机、服务、版本、暴露面、负责人、业务重要性等信息。
- **模板化导入**：资产页面提供 CSV 模板下载，并内置 JSON、nmap 示例，降低批量录入格式错误。
- **威胁关联**：基于资产服务、版本、CPE、CVE、KEV、EPSS 等信息匹配当前资产可能存在的安全威胁。
- **优先级判断**：结合业务重要性、互联网暴露、漏洞严重度、利用概率和在野利用状态生成风险评分。
- **资产详情**：可查看单个资产的服务清单、匹配漏洞、风险解释与处置建议，并支持单个或批量删除资产。
...
在左侧导航进入「资产管理」，维护当前环境的资产清单。系统会基于资产服务、版本、CPE、CVE、KEV、EPSS、暴露面和业务重要性识别风险，并在资产详情中展示匹配原因、风险评分和处置建议。

资产录入支持三种方式：

| 方式 | 适用场景 | 说明 |
|------|----------|------|
| 手动录入 | 少量关键资产、临时核查 | 输入 IP/主机名、系统、环境、重要性、服务名称、版本、端口、暴露面等信息 |
| CSV / JSON 导入 | 批量维护资产台账 | 资产页面提供 CSV 模板下载，也支持粘贴或上传 JSON |
| nmap XML 导入 | 从扫描结果生成资产 | 支持 `nmap -oX` 输出，自动解析主机、端口、服务、版本和 CPE |

服务版本建议填写明确的版本号，例如 `1.18.0`、`2.4.49`、`9.0.30`。如果来自 nmap，优先复制 `version` 字段中的版本号，不要把整段 banner 作为版本填写；banner、补充信息和人工判断可放入备注。

资产风险识别分为两层：

- **单资产识别**：在资产卡片或资产详情中点击「识别资产」，刷新该资产所有服务的 CVE 命中。
- **空间整体分析**：点击「整体分析」，生成当前资产空间的风险综合报告，输出 Top 高风险资产、暴露面判断、处置优先级和待补齐信息。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/asset-spaces` | 获取资产空间列表 |
| `POST` | `/api/asset-spaces` | 创建资产空间 |
| `GET` | `/api/assets` | 查询资产列表，支持空间、搜索、环境、重要性筛选 |
| `POST` | `/api/assets` | 手动录入或更新单个资产 |
| `GET` | `/api/assets/{host_id}` | 查看资产详情 |
| `DELETE` | `/api/assets/{host_id}` | 删除单个资产 |
| `POST` | `/api/assets/batch_delete` | 批量删除资产 |
| `POST` | `/api/assets/import/csv-text` | 导入 CSV 资产内容 |
| `POST` | `/api/assets/import/json-text` | 导入 JSON 资产内容 |
| `POST` | `/api/assets/import/nmap-text` | 导入 nmap XML 内容 |
| `GET` | `/api/assets/import/template/csv` | 下载 CSV 导入模板 |
| `POST` | `/api/assets/{host_id}/identify` | 识别单个资产的服务风险 |
| `POST` | `/api/asset-spaces/{space_id}/analyze` | 生成资产空间风险综合分析 |
...
│   │   └── ...                # 各 Agent 实现
│   ├── assets/                # 资产导入、CPE 标准化、漏洞匹配与风险评分
│   ├── db/
│   │   ├── models.py          # 数据库模型
│   │   ├── engine.py          # 异步引擎
│   │   └── repositories/      # 数据访问层
│   ├── exporters/             # 导出器（MD/PDF/STIX/CSV/Sigma/ZIP）
│   ├── routers/               # API 路由（分析/资产/数据源/历史/导出/设置等）
