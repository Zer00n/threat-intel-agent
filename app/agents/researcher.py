...
_SYSTEM_PROMPT = """You are a threat intelligence researcher. Investigate the given research question.
...
class ResearchAgent(BaseAgent):
    ...
    async def run(self, memory: Memory, question: str = "", **kwargs: Any) -> AgentResult:
        ...
        if memory.enrichment:
            enrichment_context = "\n\nAuthoritative data already collected:\n"
            for src, data in memory.enrichment.items():
                if isinstance(data, dict):
                    enrichment_context += f"- {src}: {str(data)[:500]}\n"

        system = _SYSTEM_PROMPT.format(max_rounds=settings.researcher_max_rounds)
        system += enrichment_context
